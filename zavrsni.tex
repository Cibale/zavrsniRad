...
Zanimljivo je pitanje kako je \emph{Google} toliko dobar u rezultatima koje korisnik dobije na svoj upit, točnije, kako ispravno sortira po relevantnosti stranice od važnije prema manje važnoj? Odgovor na to daje algoritam \emph{PageRank} koji je dobio ime po jednom od osnivača \emph{Googlea}, Larry Pageu. Ovo potpoglavlje analizira upravo algoritam \emph{PageRank}. U nastavku je iznesena pojednostavljena verzija algoritma, a za više informacija konzultirati \cite{pageRank}.
...
U primjeru \ref{lst:akumulatoriPrimjer} korišteni su cjelobrojni akumulatori. Osim \emph{Integer} tipa podataka, postoje akumulatori i za \emph{Long}, \emph{Double} te \emph{Float} tip podataka. Ukoliko je potrebno, moguće je kreirati i akumulator za svoj vlastiti tip podataka. Za više informacija, konzultirati \cite{workingSets}.
\chapter{Zaključak} 
...
\bibitem{marcupic}
  ...
  verzija 0.3.26
\bibitem{workingSets}
  Zaharia, Matei, et al. "Spark: Cluster Computing with Working Sets." \emph{HotCloud 10} (2010): 10-10.
\bibitem{pageRank}
	Xing, Wenpu, i Ali Ghorbani. "Weighted pagerank algorithm." \emph{Communication Networks and Services Research, 2004. Proceedings. Second Annual Conference on.} IEEE, 2004.
\end{thebibliography}
...
\texttt{java -version}. Kao rezultat se dobije trenutno instalirana verzija Jave. Ukoliko Java nije instalirana, potrebno ju je najprije instalirati. Više informacija o instalaciji se može pronaći u \cite{marcupic}.
