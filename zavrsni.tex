...
\section{Primjer: Analiza PageRank algoritma}
\subsection{Algoritam}
Zanimljivo je pitanje kako je \emph{Google} toliko dobar u rezultatima koje dobijemo na naš upit, točnije, kako ispravno sortira po relevantnosti stranice od bolje prema lošijoj? Odgovor na to daje \emph{PageRank} algoritam koji je dobio ime po jednom od osnivača \emph{Google}-a, Larry Pageu. Ovo potpoglavlje ćemo započeti kroz analizu \emph{PageRank} algoritma. Ovdje će biti iznesena pojednostavljena verzija algoritma\footnote{Zainteresiranog čitatelja se upućuje na \url{https://en.wikipedia.org/wiki/PageRank}.} koja je sasvim dovoljna za pobliže upoznavanje sa Sparkom.
...
\item Svaka stranica \emph{n} svim svojim susjedima šalje doprinos \texttt{rang(\emph{n})/brojSusjeda(\emph{n})}.
...
\end{lstlisting}
\captionof{lstlisting}{PageRank algoritam}
\vspace{5mm}
